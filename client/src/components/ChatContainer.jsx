...
const ChatContainer = ({ onProfileClick, onDetailsClick }) => {
  ...
  return selectedUser ?(
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
        ...
        <div className='flex items-center gap-3 py-3 mx-4 border-b border-stone-500'>
            {/* Removed profile picture and details icon */}
            <p className='flex-1 text-lg text-white flex items-center gap-2 cursor-pointer' onClick={onProfileClick}>
                {selectedUser.fullName}
                {onlineUsers.includes(selectedUser._id) && <span className='w-2 h-2 rounded-full bg-green-500'></span> }
            </p>
            {/* Menu icon for mobile removed */}
            ...
            {messages.map((msg, index)=>(
                <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
                    ...
                    <div className='text-center text-xs'>
                        <img src= {msg.senderId === authUser._id ? assets?.profilePic || assets.avatar_icon : selectedUser?.profilePic || assets.avatar_icon}
    ...
    <div className='flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/10 max-md:hidden'>
        ...
        <p className='text-lg font-medium text-white'>Selam - Chat anytime, anywhere</p>
    </div>
  )
}
